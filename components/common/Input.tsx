...
export const Input: React.FC<InputProps> = ({ label, id, error, className, rationale, leftIcon, helpText, isRequired, ...props }) => {
      ...
      <div className="relative">
        {leftIcon && (
          <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
            {leftIcon} {/* Icon color should be managed by its own className if needed */}
          </div>
        )}
        <input
          id={id}
          className={`w-full py-2.5 bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-lg text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:ring-2 focus:ring-purple-500 dark:focus:ring-purple-400 focus:border-purple-500 dark:focus:border-purple-400 outline-none transition duration-150 ease-in-out ${hasIcon ? 'pl-10 pr-4' : 'px-4'} ${error ? 'border-red-500 focus:ring-red-500 dark:border-red-400 dark:focus:ring-red-400' : ''} ${className}`}
          {...props}
        />
      </div>
