...
export const PersistentApiKeySection: React.FC<PersistentApiKeySectionProps> = ({
  ...
  useEffect(() => {
    ...
    // User needs to re-type if they want to change.
  }, [currentLocalKey]);
  ...
  const handleSaveLocalKey = () => {
    ...
    setInputKeyValue(''); // Clear input after saving
    setTimeout(() => setSuccessMessage(null), 3000);
  };

  return (
    <div className="bg-slate-800 p-3 shadow-md sticky top-0 z-50 border-b border-slate-700" role="region" aria-label="API 키 관리">
      <div className="container mx-auto">
        <div className="flex flex-col sm:flex-row justify-between items-center gap-2 mb-2">
            <div className="flex items-center text-slate-300 text-sm">
                {envKeyIsSet ? (
                    <>
                        <LockClosedIcon className="h-5 w-5 mr-2 text-green-400" aria-hidden="true" />
                        <span>환경 변수 API 키 사용 중 (가장 높은 우선 순위).</span>
                    </>
                ) : currentLocalKey ? (
                    <>
                        <KeyIcon className="h-5 w-5 mr-2 text-purple-400" aria-hidden="true" />
                        <span>로컬 저장소 API 키 사용 중.</span>
                    </>
                ) : (
                    <>
                        <ShieldExclamationIcon className="h-5 w-5 mr-2 text-yellow-400" aria-hidden="true" />
            ...
            <div className="flex items-center text-xs">
                {isAppUsable ? (
                    <span className="flex items-center px-2 py-0.5 rounded-full bg-green-600 text-green-100">
                        <CheckCircleIcon className="h-4 w-4 mr-1" />
                        API 키 활성
                    </span>
                ) : (
                    <span className="flex items-center px-2 py-0.5 rounded-full bg-red-600 text-red-100" role="alert">
        ...
        <div className="flex flex-col sm:flex-row items-stretch gap-2">
          <Input
            ...
            className="text-sm py-1.5 flex-grow"
          ...
          <Button 
            ...
            size="sm" 
            variant="primary" 
            className="sm:w-auto w-full whitespace-nowrap flex-shrink-0"
            leftIcon={<CheckCircleIcon className="h-4 w-4" />}
            // Allow updating local key even if env is set, it will just be overridden for current session
            // disabled={inputKeyValue.trim() === ''} 
          >
            로컬 키 저장
          </Button>
        </div>
        <p id="local-key-info" className="text-xs text-slate-500 mt-1">
            {envKeyIsSet 
